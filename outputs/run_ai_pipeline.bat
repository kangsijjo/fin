...
if errorlevel 1 ( echo [ABORT] data collection failed ^(exit !ERRORLEVEL!^) >> "%LOG%" & goto :fail )

REM ---- 2/3 build training dataset (gated) ----
echo [2/3] make_trades_history_v3.py ... >> "%LOG%"
cd /d "%OUT%"
"%PYEXE%" -u make_trades_history_v3.py >> "%LOG%" 2>&1
if errorlevel 1 ( echo [ABORT] dataset build failed ^(exit !ERRORLEVEL!^) >> "%LOG%" & goto :fail )

REM ---- 3/3 train meta-model (Optuna) (gated) ----
echo [3/3] ai_trainer_v4.py ... >> "%LOG%"
"%PYEXE%" -u ai_trainer_v4.py >> "%LOG%" 2>&1
if errorlevel 1 ( echo [ABORT] training failed ^(exit !ERRORLEVEL!^) >> "%LOG%" & goto :fail )
