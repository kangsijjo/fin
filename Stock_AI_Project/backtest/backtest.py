...
class MLStrategy(bt.Strategy):
    ...
    params = (
        ('model', None),
        ('prob_threshold', 0.6),  # 상승확률 60% 이상일 때 매수
        ('printlog', True),
    )
...
def run_backtest(ticker, sector, market='korea', cash=10000000):
    """백테스트 실행"""
    print(f"\n=== {ticker} 백테스트 시작 ===")
    ...
    try:
        ...
        print(f"샤프비율: {sharpe:.2f}" if sharpe else "샤프비율: 계산불가")
    except:
        pass
    
    try:
        drawdown = strat.analyzers.drawdown.get_analysis()['max']['drawdown']
        print(f"최대낙폭(MDD): {drawdown:.2f}%")
    except:
        pass
...
if __name__ == "__main__":
    ...
    # 삼성전자로 테스트
    run_backtest('005930', ACTIVE_SECTOR, market='korea')